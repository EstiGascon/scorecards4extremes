#!/bin/bash
#SBATCH --job-name=mars_tp_new_exps
#SBATCH --output=mars_tp_new_exps_%j.out
#SBATCH --error=mars_tp_new_exps_%j.err
#SBATCH --time=48:00:00
#SBATCH --mem=8G
#SBATCH --cpus-per-task=1

# ==============================================================================
# Retrieve total precipitation (deterministic) for new rd experiments
...
#               Step 0 not retrieved (tp=0 at init; 24h accumulation = step24-0
#               is not needed since the pipeline uses step24 directly for day 1).
# Output      : /ec/vol/destine/continuous_evaluation/precip/forecast/raw/{expver}/
#               Files named: tp_{YYYYMMDD}.grib
# ==============================================================================
...
echo "=============================================="
echo "MARS TP DET (new exps) retrieval"
...
for EXP in $EXPERIMENTS; do
    ...
    while [[ "$current" -le "$END_DATE" ]]; do
        ...
        outfile="${OUT_DIR}/tp_${current}.grib"
        ...
        if [[ -f "$outfile" ]]; then
            ...
            reqfile=$(mktemp /tmp/mars_tp_new_${EXP}_XXXXXX.req)
            cat > "$reqfile" << MARSEOF
retrieve,
  class    = rd,
  type     = fc,
