#!/bin/bash
#SBATCH --job-name=mars_10uv_det_rd
#SBATCH --output=mars_10uv_det_rd_%j.out
#SBATCH --error=mars_10uv_det_rd_%j.err
#SBATCH --time=48:00:00
#SBATCH --mem=8G
#SBATCH --cpus-per-task=1

# ==============================================================================
# Retrieve 10 m U and V wind components (deterministic) for rd experiments
...
# Output      : one GRIB file per experiment per day (10u + 10v combined)
#               /ec/vol/destine/continuous_evaluation/10wind_speed/forecast/raw/{expver}/
#
# Note: the pipeline derives 10 m wind speed from u/v components when
#       wind_speed_from_components: true  (Step 2 of run.py).
# ==============================================================================
# Usage (interactive):  bash retrieve_10uv_det_rd.sh
# Usage (batch):        sbatch retrieve_10uv_det_rd.sh
# ==============================================================================
...
echo "=============================================="
echo "MARS 10UV DET (rd) retrieval"
...
for EXP in $EXPERIMENTS; do
    ...
    while [[ "$current" -le "$END_DATE" ]]; do
        ...
        outfile="${OUT_DIR}/10uv_${EXP}_${current}.grib"
        ...
        if [[ -f "$outfile" ]]; then
            ...
            reqfile=$(mktemp /tmp/mars_10uv_det_${EXP}_XXXXXX.req)
            cat > "$reqfile" << MARSEOF
retrieve,
  class    = rd,
  type     = fc,
