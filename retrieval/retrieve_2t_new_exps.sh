#!/bin/bash
#SBATCH --job-name=mars_2t_new_exps
#SBATCH --output=mars_2t_new_exps_%j.out
#SBATCH --error=mars_2t_new_exps_%j.err
#SBATCH --time=48:00:00
#SBATCH --mem=8G
#SBATCH --cpus-per-task=1

# ==============================================================================
# Retrieve 2 m temperature (deterministic) for new rd experiments
...
# Output      : /ec/vol/destine/continuous_evaluation/2mtemp/forecast/raw/{expver}/
#               Files named: 2t_{YYYYMMDD}.grib
# ==============================================================================
...
echo "=============================================="
echo "MARS 2T DET (new exps) retrieval"
...
for EXP in $EXPERIMENTS; do
    ...
    while [[ "$current" -le "$END_DATE" ]]; do
        ...
        outfile="${OUT_DIR}/2t_${current}.grib"
        ...
        if [[ -f "$outfile" ]]; then
            ...
            reqfile=$(mktemp /tmp/mars_2t_new_${EXP}_XXXXXX.req)
            cat > "$reqfile" << MARSEOF
retrieve,
  class    = rd,
  type     = fc,
